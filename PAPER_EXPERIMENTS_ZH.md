...
要紧的是:它抵抗修复。我们尝试了**三种独立修法**——推理时给侦察降权、重新标注训练数据、在有更好动作时禁止侦察——**三种全部失败**,无法在不损伤顾问别处表现的前提下消除这个偏差。多种子检查进一步显示这个偏差的大小在不同训练种子间本身就不稳定。因此我们不把它当成"待修的缺陷",而是作为**模拟器到真实价值迁移的一个被刻画清楚的结构性局限**——它是分布偏移下*奖励模型过优化*的一个实例 [gao2023scaling]——给所有想在简化世界里训练顾问的人一个可推广的警示:凡是模拟器里少见的情形,顾问到现实中都会估错价。
...
我们明确陈述——并选择*不*以之为论文核心——以下这点:顾问对*最终结果*的好处,取决于 LLM 自己的排序本来有多好。它明显帮到弱的或没被"调教"过的 LLM(逐步 **+12 个百分点**,p = 0.02);但若我们用一个关于动作词表的明确提示去*调教* LLM,这个被调教过的 LLM 不带顾问反而比带顾问更好(该配置下 66.7% 对 39.6%)。我们完整报告这个效率倒置,而非把它藏起来。我们不以之为核心,因为 (a) 那里"被调教"的 LLM 用了作者提供的提示,这会混淆对比;(b) 它是验证器 / 过程奖励模型文献中已知现象的一个具体案例 [lightman2023verify; cobbe2021gsm8k]——一个负责*检查*的模型,被一个不再犯错的生成器架空。
