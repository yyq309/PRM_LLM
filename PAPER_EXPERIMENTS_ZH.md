...
我们把这个顾问叫做 **PRM**(*过程奖励模型*——它给每一*步*的好坏打分,而不是只看最终结果)。我们把实验组织成五个**研究问题(RQ)**;每个 RQ 在一节中回答,并对应一个创新点(C1–C3)或作为诚实局限报告:

| RQ | 问题 | 节 | 对应 |
|---|---|---|---|
| **RQ1** | 一个*只*在抽象模拟器里训练的过程评估器,能否捕捉真实的步骤质量——且无泄漏? | E.3 | C1 |
| **RQ2** | 这种过程评估能否*迁移*到真实目标,并可测量地改进逐步/阶段级的过程? | E.4 | C1、C2 |
| **RQ3** | 迁移来的过程奖励能否引导智能体走通*完整*的真实 kill chain 到 root,且其价值集中在哪? | E.5 | C3 |
| **RQ4** | 迁移来的评估器在哪里*系统性地评错*,这种失效可修复吗? | E.6 | 局限 L1 |
| **RQ5** | 这个效应是否*与模型无关*——在不同 LLM 厂商间成立吗? | E.8 | 鲁棒性 |

E.7 汇总消融与对照,抵御 RQ1–RQ3 的替代解释(泄漏、随便重排、提示词混淆);E.9 报告 proposer 条件性局限(L2);E.10 小结。RQ4 与 E.9 作为诚实局限报告,**不是**创新点。
...
| 对照 / 消融 | 要排除的替代解释 | 结果 |
|---|---|---|
| `llm_only`(去掉顾问) | 重排根本没用 | 逐步进展下降;顾问 − 基线显著,**p = 0.0012**(§E.4) |
