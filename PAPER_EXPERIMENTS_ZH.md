...
| 平均到达的 kill-chain 阶段 ↑(0=侦察 … 5=root) | **1.65** | 1.06 | — |
| 立足点/shell 到达率 ↑ | **29%** | 15% | — |
| 加权进展 ↑ | **0.59** | 0.45 | — |
| 白走步率 ↓ | **0.35** | 0.49 | — |
...
最终结果指标(整局 goal/root)在表 2b 与 E.5 给出——但上面的*过程*改进才是一个逐步奖励模型该交付的东西,而且它是**广覆盖的**(8 个指标,两个聚类显著),而非系于"某个隐晦的最终利用是否恰好在预算内拼装成功"。
