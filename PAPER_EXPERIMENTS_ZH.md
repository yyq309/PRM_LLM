...
**一句话:能——在真实机器上,让顾问重排 LLM 的动作,带来了统计显著的逐步决策质量提升。** 首先,适配器是好用的:解释器把 LLM 的自由文本映射到正确动作类型,在一个有标注的基准上准确率为 **95.5%**,在更难的留出夹具上为 **78.5%**——相比未增强基线的 **49%** 有明显提升。然后,在 15 个真实 Web 靶上,`prm` 智能体在 **54.1% 的步上向前推进,而 `llm_only` 为 42.1%——领先 +12 个百分点**,且在保守的聚类统计下显著:**p = 0.02**(多重比较校正后仍显著)。说白了:一个在低成本模拟器里、用零真实标签学到的排序感觉,可测量地改进了真实的逐步动作选择。(我们谨慎区分*逐步*质量——确有提升——与这些单服务靶上的*整局*成败——常常打平;原因在 E.5、E.9 再讲。)
...
| 对照 / 消融 | 要排除的替代解释 | 结果 |
|---|---|---|
| `llm_only`(去掉顾问) | 重排根本没用 | 逐步进展下降;顾问 − 基线显著,**p = 0.02**(§E.4) |
