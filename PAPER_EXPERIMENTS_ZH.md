...
另有两节**捍卫并压力测试**这些 RQ,而非提出新问题:E.7(消融与对照——排除泄漏、随便重排、提示词混淆)与 E.8(鲁棒性——结论在不同 LLM 厂商间成立吗?)。最后,E.6 与 E.9 报告两条诚实的**局限**(迁移来的评估器高估侦察;重排器的*结果*增益是 proposer 条件性的)——它们是诊断性分析,**不是**创新点或研究问题。E.10 小结。
...
**一句话:能——在真实机器上,让顾问重排 LLM 的动作,带来了统计显著的逐步决策质量提升。** 首先,适配器是好用的:解释器把 LLM 的自由文本映射到正确动作类型,在一个有标注的基准上准确率为 **95.5%**,在更难的留出夹具上为 **78.5%**——相比未增强基线的 **49%** 有明显提升。然后,在 16 个真实 Web 靶上,`prm` 智能体在 **52.7% 的步上向前推进,而 `llm_only` 为 37.6%——领先 +15 个百分点**,且在保守的聚类统计下显著:**p = 0.0012**(多重比较校正后仍显著)。说白了:一个在低成本模拟器里、用零真实标签学到的排序感觉,可测量地改进了真实的逐步动作选择。(整局达标率也更高——prm 31% vs `llm_only` 12%——但这个增益**集中在 proposer 彻底失败的 box**(如 SSTI、Joomla);在两臂都能解的 box 上则打平。这个*proposer 条件性*模式在 E.5、E.9 详述。)
...
**一句话:在主对比 prm vs llm_only(RQ2,§E.4)之外,这个结果还经得起我们能想到的每一个*其他*对照——它不是随便重排、不是泄漏、也不是某个 proposer 的偶然。** `llm_only`"去掉顾问"基线*就是* §E.4 的主结果,这里不再重复;表 6 的每一行隔离一个*不同的*替代解释。
...
承重的对照是**第一行**(随机重排):正因为顾问的逐步优势**并非**一致地高于随机重排,我们才**刻意不**宣称一个笼统的逐步胜利——我们宣称的是一个*阶段*与*proposer 条件*的胜利,这由整机(§E.5)与多 LLM(§E.8)结果精确刻画。

## E.8 鲁棒性——结论在不同 LLM 厂商间成立吗?
...
## E.9 局限(二)——重排器的结果增益是 proposer 条件性的
